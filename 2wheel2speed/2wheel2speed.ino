...
//unsigned char _speed8[] = {0x06, 0x00, 0x, 0x}; // -9 rpm = 1 m/s
unsigned char _speed2rpm[] = {0x06, 0x00, 0x05, 0x0b};
unsigned char _speed5rpm[] = {0x06, 0x00, 0x0e, 0x14};
unsigned char _speed5rpm2[] = {0x06,0xff,0xf2,0xf7}; // -50 rpm
...
void serialEvent()
{
  if (Serial.available())
  {
    ...
    Serial.println(data);
    if (data == '1') //forward 20 rmp
    {

      Serial2.write(mod, 4);
      Serial2.write(a_speed, 4);
      Serial2.write(still, 4);
      Serial2.write(_speed4, 4);
